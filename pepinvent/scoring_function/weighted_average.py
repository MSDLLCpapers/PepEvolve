...
class WeightedAverage:
    ...
    def _get_scoring_components(
        self, step, scoring_component_parameters: List[ScoringComponentParameters]
    ) -> List[BaseScoreComponent]:
        scoring_components = []
        for param, scoring_component in zip(scoring_component_parameters, self._scoring_components):
            if step >= param.start_step:
                scoring_components.append(scoring_component)
        return scoring_components

    def calculate_score(
        self, scoring_input: ScoringInputDTO, step: int, scoring_component_parameters: List[ScoringComponentParameters]
    ) -> FinalSummary:
        ...
        scoring_components = self._get_scoring_components(step, scoring_component_parameters)

        dtos = []
        for component in scoring_components:
            # dto: ComponentSummary = component.calculate_score(scoring_input)
            dto_list = component.calculate_score(scoring_input)
        ...
        final_summary = FinalSummary(
            total_score=total_score, scored_smiles=peptides, valid_idxs=valid_indices, scaffold_log_summary=dtos
        )
