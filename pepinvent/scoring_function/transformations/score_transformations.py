...
class TransformationFactory:
    ...
    def _default_transformation_function_registry(self) -> dict:
        ...
        transformation_list = {
            ...
            enum.REVERSE_SIGMOID_SHIFT: self.reverse_sigmoid_shift_transformation,
            enum.SIGMOID_SHIFT: self.sigmoid_shift_transformation,
            enum.SQUARE_ROOT: self.square_root_transformation,
            enum.RECIPROCAL: self.reciprocal_transformation,
        }
    ...
    def double_sigmoid(self, predictions: list, parameters: dict) -> np.array:
        ...
        def _double_sigmoid_formula(value, low, high, coef_div=100.0, coef_si=150.0, coef_se=150.0):
            try:
        ...
        transformed = [
            _double_sigmoid_formula(pred_val, _low, _high, _coef_div, _coef_si, _coef_se) for pred_val in predictions
        ]
        return np.array(transformed, dtype=np.float32)
    ...
    def custom_interpolation(self, predictions: list, parameters: dict) -> np.array:
        ...
        desirability = parameters.get(
            TransformationParametersEnum.INTERPOLATION_MAP,
            [{"origin": 0.0, "destination": 0.0}, {"origin": 1.0, "destination": 1.0}],
        )
    ...
    def reverse_sigmoid_shift_transformation(self, predictions: list, parameters: dict) -> np.array:
        _low = parameters[TransformationParametersEnum.LOW]
        _high = parameters[TransformationParametersEnum.HIGH]
        _k = parameters[TransformationParametersEnum.K]
        _shift = parameters[TransformationParametersEnum.SHIFT]

        def reverse_sigmoid_shift_formula(value, low, high, k, shift=0):
            try:
                return 1 / (1 + 10 ** (k * (value - (high + low) / 2 - shift) * 10 / (high - low)))
            except:
                return 0

        transformed = [reverse_sigmoid_shift_formula(pred_val, _low, _high, _k, _shift) for pred_val in predictions]
        return np.array(transformed, dtype=np.float32)

    def sigmoid_shift_transformation(self, predictions: list, parameters: dict) -> np.array:
        _low = parameters[TransformationParametersEnum.LOW]
        _high = parameters[TransformationParametersEnum.HIGH]
        _k = parameters[TransformationParametersEnum.K]
        _shift = parameters[TransformationParametersEnum.SHIFT]

        def sigmoid_shift_formula(value, low, high, k, shift=0):
            try:
                return 1 / (1 + 10 ** (-k * ((value - (high + low) / 2 - shift) * 10 / (high - low))))
            except:
                return 0

        transformed = [sigmoid_shift_formula(pred_val, _low, _high, _k, _shift) for pred_val in predictions]
        return np.array(transformed, dtype=np.float32)

    def square_root_transformation(self, predictions: list, parameters: dict) -> np.array:
        _high = parameters[TransformationParametersEnum.HIGH]

        def square_root_formula(value, high):
            try:
                return (value / high) ** 0.5
            except:
                return 0

        transformed = [square_root_formula(pred_val, _high) for pred_val in predictions]
        return np.array(transformed, dtype=np.float32)

    def reciprocal_transformation(self, predictions: list, parameters: dict) -> np.array:
        _k = parameters[TransformationParametersEnum.K]

        def reciprocal_formula(value, k):
            try:
                return 1 / (1 + k * value)
            except:
                return 0

        transformed = [reciprocal_formula(pred_val, _k) for pred_val in predictions]
        return np.array(transformed, dtype=np.float32)
