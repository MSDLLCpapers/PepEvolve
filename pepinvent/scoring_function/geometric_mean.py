...
class GeometricMean:
    ...
    def calculate_score(self, scoring_input: ScoringInputDTO) -> FinalSummary:
        ...
        for component in self._scoring_components:
            ...
            dto_list = component.calculate_score(cloned_scoring_input)
        ...
        peptides = [
            f"Invalid_{pep}" if idx not in valid_indices else pep for idx, pep in enumerate(scoring_input.peptides)
        ]
        final_summary = FinalSummary(
            total_score=total_score, scored_smiles=peptides, valid_idxs=valid_indices, scaffold_log_summary=dtos, input_chuckles=cloned_scoring_input.chuckles
        )
    ...
    def _strip_invalids(self, scoring_input: ScoringInputDTO, valid_indices) -> ScoringInputDTO:
        ...
        scoring_input.chuckles = [scoring_input.chuckles[idx] for idx in valid_indices]
