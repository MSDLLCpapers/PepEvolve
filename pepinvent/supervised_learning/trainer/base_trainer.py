...
from pepinvent.supervised_learning.trainer.dynamic_mask_dataset import DynamicMaskDataset
from pepinvent.supervised_learning.trainer.dynamic_mask_shift_dataset import DynamicMaskShiftDataset
...
class BaseTrainer(ABC):
    ...
    def initialize_dynamic_mask_dataloader(self, data_path, batch_size, vocab):
        data = pd.read_csv(data_path, sep=",")
        dataset = DynamicMaskDataset(data=data, vocabulary=vocab, tokenizer=SMILESTokenizer())
        dataloader = DataLoader(dataset, batch_size,
                                                 drop_last=self._config.drop_last_batch,
                                                 shuffle=self._config.shuffle_each_epoch,
                                                 collate_fn=Dataset.collate_fn,
    ...
    def initialize_dynamic_mask_shift_dataloader(self, data_path, batch_size, vocab):
        data = pd.read_csv(data_path, sep=",")
        dataset = DynamicMaskShiftDataset(data=data, vocabulary=vocab, tokenizer=SMILESTokenizer())
        dataloader = DataLoader(dataset, batch_size,
                                                 drop_last=self._config.drop_last_batch,
                                                 shuffle=self._config.shuffle_each_epoch,
                                                 collate_fn=Dataset.collate_fn,
