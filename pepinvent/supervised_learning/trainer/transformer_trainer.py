...
from pepinvent.supervised_learning.trainer.dynamic_create_vocabulary import DynamicVocabularyMaker
...
class TransformerTrainer(BaseTrainer):
    ...
    def execute(self):
        ...
        vocabulary_maker = DynamicVocabularyMaker()
        vocab = vocabulary_maker.create_vocabulary(self._config.training_data_path, self._config.validation_data_path)
        vocab_size = len(vocab.tokens())

        # Data loader
        if self._config.data_mode == 'mask': 
            dataloader_train = self.initialize_dynamic_mask_dataloader(self._config.training_data_path, self._config.batch_size, vocab)
        elif self._config.data_mode == 'mask_shift':
            dataloader_train = self.initialize_dynamic_mask_shift_dataloader(self._config.training_data_path, self._config.batch_size, vocab)
        else: 
        ...
        dataloader_validation2 = self.initialize_dataloader(self._config.validation2_data_path, self._config.batch_size, vocab)
        ...
        for epoch in range(self._config.starting_epoch, self._config.starting_epoch + self._config.num_epoch):
            ...
            loss_epoch_validation2, _, _, _, _ = self.validation_stat(
                dataloader_validation2, model.network,
                SimpleLossCompute(model.network.generator, criterion, None),
                device, vocab)
            
            self.LOG.info("Validation end")
            self.LOG.info(
                "Train loss, Validation loss, Validation loss 2, identity_accuracy, token_accuracy, sim_avg: {}, {}, {}, {}, {}, {}".format(
                    round(loss_epoch_train, 5), round(loss_epoch_validation, 5), round(loss_epoch_validation2, 5),
