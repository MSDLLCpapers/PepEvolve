...
class Sampling:
    ...
    def _create_report(self, results: Dict[str, List[SampledSequencesDTO]]) -> pd.DataFrame:
        # Collect rows efficiently in a list first
        rows = []
        ...
        # Create the DataFrame in one go (much faster than append)
        dataframe = pd.DataFrame(rows, columns=['Input', 'Output', 'NLLs'])

        # Expand the list of outputs into separate columns
        if len(rows) > 0:
            column_labels = [f'Generated_smi_{x}' for x in range(1, len(rows[0]["Output"]) + 1)]
