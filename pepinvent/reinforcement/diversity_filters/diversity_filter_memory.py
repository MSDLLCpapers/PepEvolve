...
class DiversityFilterMemory:
    ...
    def __init__(self):
        ...
        df_dict = {"Period": [], "Step": [], "Mask Position": [], "Input_CHUCKLES": [], "Monomers": [], "CHUCKLES": [], "SMILES": [], "Scaffold": []}
        self._memory_dataframe = pd.DataFrame(df_dict)

    def update(self, indx: int, score: float, smile: str, sampled_sequences: str, components: List[ComponentSummary], step: int, scaffold: str = '', input_chuckles: str = '', monomers: str = '', period: str = '', mask_pos: int = None):
        ...
        if not self.smiles_exists(smile):
            self._add_to_memory_dataframe(step, smile, sampled_sequences, component_scores, scaffold, input_chuckles, monomers, period, mask_pos)

    def _add_to_memory_dataframe(self, step: int, smile: str, sampled_sequences: str, component_scores: Dict, scaffold: str = '', input_chuckles: str = '', monomers: str = '', period: str = '', mask_pos: int = None):
        data = []
        headers = []
        ...
        headers.append("Step")
        data.append(step)
        headers.append("Mask Position") 
        data.append(mask_pos)
        headers.append("Input_CHUCKLES")
        data.append(input_chuckles)
        headers.append("Monomers")
