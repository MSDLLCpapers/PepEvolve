...
class BaseDiversityFilter(abc.ABC):
    ...
    @abc.abstractmethod
    def update_score(self, score_summary: FinalSummary, sampled_sequeces: List[SampledSequencesDTO], step, period, mask_pos) -> np.array:
    ...
    def _add_to_memory(self, indx: int, score, smile, sampled_sequence, components: List[ComponentSummary], step, scaffold: str = '', input_chuckles: str = '', monomers: str = '', period: str = '', mask_pos: int = None):
        self._diversity_filter_memory.update(indx, score, smile, sampled_sequence, components, step, scaffold, input_chuckles, monomers, period, mask_pos)
