...
class LocalReinforcementLogger(BaseReinforcementLogger):
    def __init__(self, rl_config: LoggingConfig):
        ...
        self._summary_writer = SummaryWriter(log_dir=f'{self._log_config.logging_path}')
    ...
    def timestep_report(self, agent_idx, start_time, n_steps, step, score_summary: FinalSummary,
                        agent_likelihood: torch.tensor, prior_likelihood: torch.tensor,
                        augmented_likelihood: torch.tensor, report: List[OutputDTO]):

        message = self._console_message_formatter.create(agent_idx, start_time, n_steps, step, score_summary.scored_smiles, score_summary.total_score.mean(), score_summary,
                                                         score_summary.total_score, agent_likelihood, prior_likelihood, augmented_likelihood)
        self._logger.info(message)
        self._tensorboard_report(agent_idx, step, score_summary.scored_smiles, score_summary.total_score, score_summary, agent_likelihood, prior_likelihood,
                                 augmented_likelihood, report)

    def save_final_state(self, output: pd.DataFrame, name: str = 'results.csv'):
        output.to_csv(os.path.join(self._log_config.result_path, name))
        # self._summary_writer.close()

    def _tensorboard_report(self, agent_idx, step, smiles, score, score_summary: FinalSummary, agent_likelihood, prior_likelihood,
                            augmented_likelihood, outputs: List[OutputDTO]):
        self._summary_writer.add_scalars("nll/avg", {
            "prior": prior_likelihood.mean(),
            f"augmented_{agent_idx}": augmented_likelihood.mean(),
            f"agent_{agent_idx}": agent_likelihood.mean()
        }, step)
        self._total_score = np.mean(score)
        self._fraction_valid = lu.fraction_valid_smiles(smiles)
        for i, log in enumerate(score_summary.profile):
            self._summary_writer.add_scalar(score_summary.profile[i].name, np.mean(score_summary.profile[i].score),
                                            step)
        self._summary_writer.add_scalar(f"Valid SMILES/agent_{agent_idx}", self._fraction_valid, step)
        self._summary_writer.add_scalar(f"Average score", self._total_score, step)
        self._summary_writer.add_scalar(f"True score", np.sum(score) / 128, step)
        self._summary_writer.add_scalar(f"Average score/agent_{agent_idx}", self._total_score, step)
